...
export function FormEjercicios() {
  ...
  const id = routeParams.idEjercicio || null;
  const esCrear = !id;
  // Valores a precarga al actualizar
  const [values, setValues] = useState(null);
  // Esquema de validación
  const ejerciciosSchema = yup.object({
    nombre: yup
      .string()
      .required('El título es requerido')
      .min(3, 'El título debe tener 3 caracteres'),
    descripcion: yup.string().required('Los minutos son requerido'),
    equipamento: yup.string().required('Los minutos son requerido'),
    rutinas: yup.array().typeError('Seleccione un ejercicio'),
  });
  const {
    control,
    handleSubmit,
    ...
    resolver: yupResolver(ejerciciosSchema),
  });
  // useFieldArray:
  ...
  useEffect(() => {
    if(start){
      if(esCrear){
        //Crear pelicula
        EjerciciosService.crearEjercicio(formData)
        .then(response => {
        ...
        //Modificar pelicula
        EjerciciosService.actualizarEjercicio(formData)
        .then(response => {
          console.log(response)
            setResponseData(response.data.results)
            setError(response.error)
  ...
  useEffect(() => {
    if(id!=undefined && !isNaN(Number(id))){
      EjerciciosService.obtenerEjercicioFormPorId(id)
    .then(response => {
      console.log(response)
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    ...
                    id='nombreEjercicio'
                    label='Nombre'
                    error={Boolean(errors.nombreEjercicio)}
                    helperText={errors.nombreEjercicio ? errors.nombreEjercicio.message : ' '}
                  />
                )}
              />
            </FormControl>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    ...
                    helperText={errors.equipamento ? errors.equipamento.message : ' '}
                  />
                )}
              />
            </FormControl>
              ...
              {loadedRutinas &&
                ...
                fields.map((field, index) => (
                  <RutinasForm
                    ...
                    key={index}
                    index={index}
                    onRemove={removerRutina}
                    control={control}
                    onChange={(e) => setValue('rutinas', e.target.value, { shouldValidate: true })}
                    disableRemoveButton={fields.length === 1}
                  />
                ))}
              <FormHelperText sx={{ color: '#d32f2f' }}>
                {errors.rutinas ? errors.rutinas.message : ' '}
              </FormHelperText>
            </FormControl>
          </Grid>
          <Grid item xs={12} sm={12}>
