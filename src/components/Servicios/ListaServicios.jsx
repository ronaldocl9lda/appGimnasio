...
export function ListaServicios () {
  ...
  return (
    <Grid container sx={{ p: 2 }} spacing={3}>
      ...
      {data && data.map((item)=>( 
          <Grid item xs={4} key={item.idServicio}  >
            <Card>
              <CardHeader
                sx={{
                  p: 0,
              ...
              <CardContent>
                ...
                <Typography variant='body2' color='text.secondary'>
                  <FitnessCenterIcon /> {item.foto} Foto
                </Typography>
              </CardContent>
              <CardActions
                disableSpacing
              ...
              >
                <IconButton component={Link} to={`/servicios/${item.idServicio}`} aria-label='Detalle' sx={{ ml: 'auto' }}>
                  <Info/>
                </IconButton>
              </CardActions>
            </Card>
